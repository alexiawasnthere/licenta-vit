...
def load_frame(
    ...
    return decode_and_resize_jpeg(image_bytes, img_size, training)
...
def load_clip_tf(
    ...
    frames = tf.map_fn(
        lambda p: load_frame(p, img_size, False),  # fără augment aici
        paths,
...
def load_clip_and_label_tf(
    ...
    clip = load_clip_tf(data_root, num_frames, img_size, split, video_id, training)
