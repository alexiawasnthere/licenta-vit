...
def decode_and_resize_jpeg(image_bytes: tf.Tensor, 
                           img_size: int
                           ) -> tf.Tensor:
    img = tf.image.decode_jpeg(image_bytes, channels=3)   # uint8
    img = tf.image.convert_image_dtype(img, tf.float32)   # float32
    img = tf.image.resize(img, (img_size, img_size))      # float32
    return img

def load_frame(path: tf.Tensor, 
               img_size: int
               ) -> tf.Tensor:
    # citeste un jpeg de pe disc si il preproceseaza
    image_bytes = tf.io.read_file(path)
    return decode_and_resize_jpeg(image_bytes, img_size)


def load_clip(data_root: Path, 
              num_frames: int, 
              img_size: int, 
              video_id: int, 
              split: SplitName
    ...
    frames = [load_frame(tf.constant(p), img_size) for p in frame_strs]
    return tf.stack(frames, axis=0)  # (t, h, w, c)


def load_clip_and_label(data_root: Path, 
                        num_frames: int, 
                        img_size: int, 
                        video_id: int, 
                        label_id: int, 
...
def load_clip_tf(data_root: str, 
                 num_frames: int, 
                 img_size: int, 
                 split: str, 
                 video_id: tf.Tensor
    ...
    frames = tf.map_fn(
        lambda p: load_frame(p, img_size),
        paths,
...
def load_clip_and_label_tf(data_root: str, 
                           num_frames: int, 
                           img_size: int, 
                           split: str, 
                           video_id: tf.Tensor, 
    ...
    clip = load_clip_tf(data_root, num_frames, img_size, split, video_id)
